...
# ⚡ Bolt: Added optional output variable parameter to avoid spawning subshells in loops
human_readable() {
  local kb="$1"
  local outvar="${2:-}"
  local res
  ...
  while IFS= read -r line; do
    case "$line" in
      CLEAN_STEP:*)
        ...
        [[ "$freed" =~ ^-?[0-9]+$ ]] || freed=0
        local fname
        # ⚡ Bolt: Use refactored function with output variable to prevent subshell spawning
